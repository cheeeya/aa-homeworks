...
class MetaCorgiSnacks
  ...
  #   snack_name = name.to_s
  #   snack_box_info = "get_" + snack_name + "_info"
  #   snack_box_tastiness = "get_" + snack_name + "_tastiness"
  #   info = @snack_box.send(snack_box_info.to_sym, @box_id)
  #   tastiness = @snack_box.send(snack_box_tastiness.to_sym, @box_id)
  #   result = "#{snack_name.capitalize}: #{info}: #{tastiness} "
  #   tastiness > 30 ? "* #{result}" : result
  # end
  ...
  def self.define_snack(name)
    define_method(name) do
      snack_box_info = "get_" + name + "_info"
      snack_box_tastiness = "get_" + name + "_tastiness"
      info = @snack_box.send(snack_box_info.to_sym, @box_id)
      tastiness = @snack_box.send(snack_box_tastiness.to_sym, @box_id)
